...
// count single/double/triple clicks
#include "Clicks.h"
const int buttonPin = 3;
Clicks clickCounter(buttonPin);
int clicks;
...
void initLoop(){
  if (runOnce){
    ...
    display.println(F("GoleBox"));

    display.setTextSize(1);
    display.setTextColor(SSD1306_BLACK, SSD1306_WHITE); // Draw 'inverse' text
    display.println("  Addict to your goals  ");
    display.display();
  }
...
void goalLoop(){
  ...
  if (clicks == 1) {
    goleDone[goleInd] = true;
  }

  if (clicks == 1 || clicks == 2) {
    goleInd = (goleInd + 1) % NUM_GOLES;
  }

  if (clicks == 3) {
    // goleInd = (goleInd + NUM_GOLES - 1) % NUM_GOLES;
    initAnimateLoop();
  }
}
...
ull startAnimate, animateLen = 1000;
...
void loop() {
  clicks = clickCounter.getClicks();

  switch(state) {
    case Init: initLoop(); break;
    case Goal: goalLoop(); break;
    case Animate: animateLoop(); break;
  }
}
